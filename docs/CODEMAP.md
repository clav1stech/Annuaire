...
- `name_search_app.py` — app Streamlit secondaire : recherche floue de nom sur les parquets SIRENE, à partir d'une feuille d'anomalies produite par `app.py`.
...
- `requirements.txt`, `create_venv.bat`, `run_app.bat`, `run_name_search.bat` — installation et lancement (Windows).
...
`io_utils` / `sirene_queries` (accès données) → `pipeline` (métier) → `export_utils` / `ui_helpers` (présentation) → `app.py` / `name_search_app.py` (entrypoints).
